...
/**
 * กุญแจของการผูกบัญชีไลน์ในตาราง line_accounts
 *
 * ทุกระบบที่อยู่บน LINE OA เดียวกันของบริษัทใช้ค่าเดียวกันคือ "core" — พนักงานลงทะเบียนครั้งเดียว
 * แล้วใช้ได้ทุกระบบ (แจ้งปัญหา · จองคิวนวด · ระบบอื่นในอนาคต) เพราะ userId ของไลน์ผูกกับ OA
 * ไม่ได้ผูกกับตัวระบบ อยู่ OA เดียวกันจึงเป็น userId เดียวกันอยู่แล้ว
 *
 * ค่านี้จะต่างกันก็ต่อเมื่อระบบใหม่ไปอยู่คนละ OA ซึ่งตอนนั้น userId จะเป็นคนละค่าจริง ๆ
 */
export const CHANNEL_KEY = "core";

/**
 * กุญแจรุ่นก่อนที่ยังต้องอ่านเจอ — ตอนแรกระบบแจ้งปัญหาใช้ "report" เป็นของตัวเอง
 * เก็บไว้เพื่อให้คนที่ผูกบัญชีไว้แล้วไม่ต้องลงทะเบียนใหม่ ไม่ว่าจะ deploy โค้ดก่อนหรือรัน SQL ก่อน
 * ลบทิ้งได้เมื่อรัน db/add-core-channel.sql แล้วและไม่มีแถวไหนเหลือค่าเดิม
 */
export const CHANNEL_KEYS_READ = ["core", "report"];
 ...
 *
 * จุดประสงค์หลักไม่ใช่การเก็บสถิติ แต่คือการทำให้คำขอบคุณเดินทางไปถึงคนที่ลงมือทำจริง ๆ
 * งานซ่อมบำรุงเป็นงานที่ไม่มีใครพูดถึงตอนทุกอย่างปกติ และถูกพูดถึงเฉพาะตอนมีปัญหา
 *
 * เป็นตัวเลือกให้กดทั้งหมด ไม่มีช่องให้พิมพ์ เพราะคนกดคือผู้แจ้งที่งานของตัวเองจบไปแล้ว
 * ถ้าต้องพิมพ์จะไม่มีใครตอบ
 */
...
/** คำชมสำหรับงานที่ทำได้ดี (4-5 ดาว) — ส่งต่อให้ผู้รับผิดชอบอ่านเอง */
export const PRAISE_CHIPS = [
  "เสร็จไวมาก",
  "แก้ปัญหาได้ดี",
  "ใส่ใจดูแลดี",
  "อธิบายชัดเจน",
...
/** สิ่งที่ควรปรับปรุง (1-3 ดาว) — เก็บไว้ในระบบ ไม่ส่งต่อให้ผู้รับผิดชอบ */
export const IMPROVE_CHIPS = [
  "ใช้เวลานานเกินไป",
  "แก้แล้วยังไม่หาย",
  "ติดต่อยาก",
  "ไม่ได้แจ้งความคืบหน้า",
];
