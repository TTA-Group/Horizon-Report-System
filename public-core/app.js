...
function renderMenuStatus(r) {
  ...
  if (r.defaultMenu && r.defaultMenu.ok) {
    rows.push(
      r.defaultMenu.correct
        ? checkRow("good", `เมนูตั้งต้นของ OA ถูกต้องแล้ว — <b>${esc(r.defaultMenu.name || "ไม่ทราบชื่อ")}</b>`,
            "ทุกคนใน OA ที่ระบบมองไม่เห็น จะได้เมนูนี้")
        : r.defaultMenu.id
          ? checkRow("bad", `เมนูตั้งต้นของ OA ยังเป็นใบอื่นอยู่ — <b>${esc(r.defaultMenu.name || "ไม่ทราบชื่อ")}</b>`,
              `<span class="mono">${esc(r.defaultMenu.id)}</span><br>` +
              "ควรเป็นเมนูของคนที่ยังไม่ลงทะเบียน · กดปุ่มด้านล่างแล้วระบบจะตั้งให้เอง")
          : checkRow("warn", "ยังไม่ได้ตั้งเมนูตั้งต้นของ OA",
              "คนที่ระบบมองไม่เห็นจะไม่มีเมนูเลย · กดปุ่มด้านล่างแล้วระบบจะตั้งให้เอง"),
    );
  }
  ...
  $("#menu-body").innerHTML = `
    ...
    <p class="hintnote">
      ${r.ready
        ? `กดครั้งเดียวทำสองอย่าง —
           <b>ตั้งเมนูตั้งต้นของ OA</b> ซึ่งถึงทุกคนใน OA พร้อมกัน แม้คนที่ระบบไม่รู้จัก ·
           แล้ว<b>ไล่ผูกรายคน</b>ให้อีก ${r.people} บัญชีที่ระบบรู้จัก ตามสถานะของแต่ละคน
...
async function applyRichMenus() {
  ...
  try {
    ...
    box.innerHTML =
      `<p class="hintnote">
         <b>เสร็จแล้ว</b><br>
         • ตั้งเมนูตั้งต้นของ OA แล้ว — <b>ทุกคนใน OA</b> ที่ไม่มีเมนูผูกไว้เป็นรายคน เปลี่ยนตามทันที
           ไม่ว่าระบบจะรู้จักหรือไม่<br>
         • ผูกเมนูรายคนให้อีก <b>${total}</b> บัญชีที่ระบบรู้จัก${failed ? ` (ไม่สำเร็จ ${failed} บัญชี — มักเป็นคนที่บล็อกหรือลบ OA ไปแล้ว)` : ""}<br>
