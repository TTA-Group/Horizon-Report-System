...
function renderMenuStatus(r) {
    ...
    rows.push(
      r.defaultMenu.id
        ? checkRow("warn", `มีเมนูตั้งต้นของ OA อยู่ — <b>${esc(r.defaultMenu.name || "ไม่ทราบชื่อ")}</b>`,
            ...
            "ระบบนี้ตั้งใจไม่ใช้เมนูตั้งต้น เพราะคนใหม่จะเห็นเมนูใช้งานตั้งแต่ยังไม่ลงทะเบียน " +
            "และคนที่ลาออกแล้วซึ่งถูกถอดเมนูออก จะตกกลับไปเห็นเมนูตั้งต้นแทนที่จะไม่มีเมนู" +
            '<br><button class="linkbtn" id="menu-cleardef">ถอดเมนูตั้งต้นออก</button>')
  ...
  $("#menu-body").innerHTML = `
    ...
    <p class="hintnote">
      ${r.ready
        ? `${r.followers && r.followers.ok
             ? "ดึงรายชื่อเพื่อนทั้งหมดจาก LINE มาใหม่ก่อน แล้วไล่ตั้งให้ทุกคน"
             : `ไปถึง <b>${r.people}</b> บัญชีที่ระบบรู้จัก (ขอรายชื่อเพื่อนทั้งหมดจาก LINE ไม่ได้)`}
           ตามสถานะของแต่ละคน —
