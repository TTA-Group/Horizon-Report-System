...
function renderMenuStatus(r) {
  ...
  $("#menu-body").innerHTML = `
    ...
    <button class="send" id="menu-apply"${r.ready ? "" : " disabled"}>1 · เปลี่ยนเมนูหลักให้ทุกคน</button>
    <p class="hintnote">
      ${r.ready
    ...
    <button class="send" id="menu-apply-one" style="margin-top:14px"${r.ready ? "" : " disabled"}>2 · เปลี่ยนให้เฉพาะคน</button>
...
async function applyRichMenus() {
  ...
  try {
    ...
    let defaultSet = false, defaultBlocked = false, blankMade = false;
    for (;;) {
      const r = await api("/api/admin/richmenu", { method: "POST", body: { after } });
      // ชุดแรกเป็นชุดเดียวที่แตะเมนูตั้งต้น เก็บผลไว้บอกตอนจบ
      if (rounds === 0) {
        defaultSet = r.defaultSet === true;
        defaultBlocked = r.defaultBlocked === true;
        blankMade = r.blankMade === true;
      }
      total += r.processed;
      failed += r.failed;
      rounds += 1;
    ...
    // บอกตามที่เกิดขึ้นจริง — เดิมเขียนตายตัวว่า "ถึงทุกคนแล้ว" ทั้งที่บางครั้งตั้งเมนูตั้งต้นไม่ได้
    // คนอ่านจึงเชื่อว่าเรียบร้อย แล้วไปเจอเองทีหลังว่าคนที่ไม่เคยทักแชทไม่มีเมนู
    box.innerHTML =
      `<p class="hintnote">
         <b>เสร็จแล้ว</b><br>
         ${defaultSet
           ? "• คนที่<b>ไม่เคยทักแชท</b>และระบบไม่รู้จัก ได้เมนูหลักแล้วผ่านเมนูตั้งต้นของ OA<br>"
           : defaultBlocked
             ? `<span style="color:#B23A21">• <b>ยังไปไม่ถึงคนที่ไม่เคยทักแชท</b> — สร้างเมนูว่างเปล่าไม่สำเร็จ
                จึงยังตั้งเมนูตั้งต้นให้ไม่ได้ (ตั้งไปคนที่ถูกถอดจะกลับมาเห็นเมนูหลัก)<br>
                ดูสาเหตุได้ที่กล่องผลตรวจด้านบน</span><br>`
             : "• เมนูตั้งต้นของ OA เป็นเมนูหลักอยู่แล้ว คนที่ไม่เคยทักแชทจึงได้เมนูหลักด้วย<br>"}
         ${blankMade ? "• สร้างเมนูว่างเปล่าให้อัตโนมัติแล้ว สำหรับคนที่ถูกถอด<br>" : ""}
