...
function renderMenuStatus(r) {
  ...
  if (r.defaultMenu && r.defaultMenu.ok) {
    if (r.defaultMenu.correct) {
      note("good");
      rows.push(checkRow("good", `พนักงานเก่าที่ระบบไม่รู้จัก ก็ได้เมนูหลัก — <b>${esc(r.defaultMenu.name || "ไม่ทราบชื่อ")}</b>`,
        "ตั้งเป็นเมนูตั้งต้นของ OA ไว้ คนที่เป็นเพื่อนอยู่แล้วแต่ไม่เคยทักแชทและไม่เคยลงทะเบียน " +
        "จึงได้เมนูหลักด้วย · ส่วนคนที่เพิ่งแอดเข้ามาใหม่ได้เมนูลงทะเบียนตามปกติ"));
      ...
      rows.push(checkRow("bad", "ยังไม่ได้ตั้งเมนูตั้งต้นของ OA",
        "พนักงานเก่าที่เป็นเพื่อนอยู่แล้วแต่ไม่เคยทักแชทและไม่เคยลงทะเบียน จะไม่มีเมนูเลย · " +
  ...
  $("#menu-body").innerHTML = `
    ...
    <p class="hintnote">
      ${r.ready
        ? `พนักงานที่เป็นเพื่อนอยู่ตอนนี้ได้<b>เมนูหลักเหมือนกันหมด</b> ไม่ว่าจะลงทะเบียนแล้วหรือยัง` +
          (excluded.length ? `<br><b>ข้าม ${excluded.length} คน</b>ที่ถอดเมนูไว้` : "") +
          `<br>คนที่ถูกระงับสิทธิ์ได้เมนูลงทะเบียนแทน` +
          `<br>คนที่<b>แอดเข้ามาใหม่หลังจากนี้</b>ได้เมนูลงทะเบียนเองอัตโนมัติ`
    ...
    <button class="send" id="menu-unlink" style="margin-top:14px">3 · ถอดเมนูหลักเฉพาะบางคน</button>
    <p class="hintnote">คนที่ถูกถอดจะเหลือแค่<b>เมนูลงทะเบียน</b> ไม่ใช่เมนูหลัก
      และปุ่มข้อ 1 จะข้ามคนนี้ไปตลอดจนกว่าจะกดข้อ 2 คืนให้<br>
      (ให้ไม่มีเมนูเลยไม่ได้ เพราะ OA ตั้งเมนูตั้งต้นไว้ให้พนักงานเก่าที่ระบบมองไม่เห็น)</p>
...
async function menuActFor(employeeId, name) {
  ...
  const ok = await confirmDialog({
    ...
    message: apply
      ...
      : "คนนี้จะเหลือแค่เมนูลงทะเบียน ไม่ได้เมนูหลักอีก และปุ่มข้อ 1 จะข้ามคนนี้ไปตลอด " +
        "จนกว่าจะกดปุ่ม “เปลี่ยนเฉพาะบุคคล” คืนให้ · เรื่องที่เคยแจ้งและคิวที่จองไว้ยังอยู่ครบ",
